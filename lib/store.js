...
function readDB() {
  ...
  if (!db.data || typeof db.data !== "object") { db.data = {}; dirty = true; }
  // セッション署名用のシークレットは一度だけ生成して保存する（毎回変わるとログインが切れてしまう）。
  if (!db.secret) { db.secret = crypto.randomBytes(32).toString("hex"); dirty = true; }
  if (dirty) writeDB(db);
  return db;
}
