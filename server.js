...
function overlapHours(start, end, rangeStart, rangeEnd) {
  return Math.max(0, (Math.min(end, rangeEnd) - Math.max(start, rangeStart)) / 3600000);
}

function nightHours(startValue, endValue) {
  ...
  while (cursor < end) {
    ...
    const nightEnd = new Date(cursor); nightEnd.setDate(nightEnd.getDate() + 1); nightEnd.setHours(5, 0, 0, 0);
    total += overlapHours(start, end, nightStart, nightEnd);
    cursor.setDate(cursor.getDate() + 1);
  }
  return total;
}

function workSummary(event, settings, actual = false) {
  const startValue = actual ? event.actualStartAt : event.startAt;
  const endValue = actual ? event.actualEndAt : event.endAt;
  if (!startValue || !endValue) return { hours: 0, nightHours: 0, breakHours: 0, pay: 0 };
  const start = new Date(startValue);
  const end = new Date(endValue);
  const breaks = actual ? (event.breaks || []).filter((item) => item.startAt && item.endAt) : [];
  const breakHours = breaks.reduce((sum, item) => sum + overlapHours(new Date(item.startAt), new Date(item.endAt), start, end), 0);
  const breakNightHours = breaks.reduce((sum, item) => sum + nightHours(item.startAt, item.endAt), 0);
  const hours = Math.max(0, overlapHours(start, end, start, end) - breakHours);
  const deepNight = Math.max(0, nightHours(start, end) - breakNightHours);
  const pay = hours * settings.hourlyWage + deepNight * settings.hourlyWage * settings.nightBonusRate / 100 + settings.transportPerShift;
  return { hours, nightHours: deepNight, breakHours, pay: Math.round(pay) };
}

function currentLocalDateTime() {
  return new Intl.DateTimeFormat("sv-SE", { timeZone: "Asia/Tokyo", year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false }).format(new Date()).replace(" ", "T");
}
...
app.post("/events", async (req, res) => {
  ...
  const existing = req.body.id ? await req.store.find("events", req.body.id) : null;
...
app.post("/events/:id/clock/:action", async (req, res) => {
  assertInput(validId(req.params.id), "予定IDが正しくありません。");
  const event = await req.store.find("events", req.params.id);
  if (!event || event.category !== "アルバイト") return res.sendStatus(404);
  const now = currentLocalDateTime();
  const breaks = Array.isArray(event.breaks) ? [...event.breaks] : [];
  if (req.params.action === "planned") {
    event.actualStartAt = event.startAt;
    event.actualEndAt = event.endAt;
    event.breakStartedAt = null;
    breaks.length = 0;
  } else if (req.params.action === "start" && !event.actualStartAt) event.actualStartAt = now;
  else if (req.params.action === "break-start" && event.actualStartAt && !event.actualEndAt && !event.breakStartedAt) event.breakStartedAt = now;
  else if (req.params.action === "break-end" && event.breakStartedAt && !event.actualEndAt) {
    breaks.push({ startAt: event.breakStartedAt, endAt: now });
    event.breakStartedAt = null;
  } else if (req.params.action === "end" && event.actualStartAt && !event.actualEndAt) {
    if (event.breakStartedAt) breaks.push({ startAt: event.breakStartedAt, endAt: now });
    event.breakStartedAt = null;
    event.actualEndAt = now;
  } else return res.status(400).send("現在の打刻状態では、この操作はできません。");
  event.breaks = breaks;
  await req.store.save("events", event);
  res.redirect("/work");
});
...
app.get("/salary", async (req, res) => {
  const data = await req.store.read();
  const requestedMonth = /^\d{4}-\d{2}$/.test(req.query.month || "") ? req.query.month : monthKey(new Date());
  const shifts = data.events.filter((item) => item.category === "アルバイト" && item.startAt.slice(0, 7) === requestedMonth)
    .sort((a, b) => new Date(a.startAt) - new Date(b.startAt))
    .map((item) => ({ ...item, forecast: workSummary(item, data.settings), actual: workSummary(item, data.settings, true) }));
  const total = (key) => shifts.reduce((sum, item) => sum + item[key].pay, 0);
  const totalHours = (key) => shifts.reduce((sum, item) => sum + item[key].hours, 0);
  const base = new Date(`${requestedMonth}-01T00:00:00`);
  const prev = monthKey(new Date(base.getFullYear(), base.getMonth() - 1, 1));
  const next = monthKey(new Date(base.getFullYear(), base.getMonth() + 1, 1));
  res.render("salary", { title: "予想給料", month: requestedMonth, shifts, forecastPay: total("forecast"), actualPay: total("actual"), forecastHours: totalHours("forecast"), actualHours: totalHours("actual"), prev, next });
});

app.get("/work", async (req, res) => {
  const shifts = (await req.store.list("events"))
    .filter((item) => item.category === "アルバイト")
    .sort((a, b) => new Date(a.startAt) - new Date(b.startAt));
  res.render("work", { title: "バイト", shifts });
});

app.get("/settings", async (req, res) => res.render("settings", { title: "設定", periods: (await req.store.read()).settings.periods }));
app.post("/settings/salary", async (req, res) => {
  const hourlyWage = Number(req.body.hourlyWage);
  const nightBonusRate = Number(req.body.nightBonusRate);
  const transportPerShift = Number(req.body.transportPerShift);
  if (![hourlyWage, nightBonusRate, transportPerShift].every(Number.isFinite) || hourlyWage < 0 || nightBonusRate < 0 || transportPerShift < 0) {
    return res.status(400).send("給料設定には0以上の数字を入力してください。");
  }
  await req.store.saveSettings({ hourlyWage, nightBonusRate, transportPerShift });
  res.redirect("/salary");
});
